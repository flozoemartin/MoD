...
* This is a revised do-file for the data analysis performed for my MSc dissertation - I have knitted together the useful parts of the do-files I had for analysis, 
* in the order in which the information is required for the manuscript. I will also be adding the revised regressions (ordinal logistic) and the imputation model 
* for missing data.

cd "/Users/ti19522/OneDrive - University of Bristol/Documents/PhD/Year 1/ALSPAC MoD Study/Data"
use flo_dataset.dta, clear

* First in the manuscript is the flow diagram of participants through the study - important to know who is being lost at each stage. qlet = A ensures each mother 
* is only counted once and mod < 6 excludes women with a mode of delivery coded as other. This can be added to as exclusion criteria is determined
...
* Now I have added the women who are coded in mod as other but caesarean as emergency CS to the CS group in mod - now the groups I have aren't "contaminated" 
* with multiple methods of birth
tab mod caesarean if qlet == "A" 
recode mod (6 = 3) if mod ==6 & caesarean ==2 
tab mod caesarean if qlet == "A"

* Now I need to look at the distribution of complete case women at each timepoint for each outcome stratified by each exposure comparison (VD vs CS, non-ins vs 
* ins, elective vs emergency)
drop if mod ==6 
...
xlabel(0(1)5, valuelabel labsize(1.2)) title(33 months postpartum, size(3))
...
xlabel(0(1)5, valuelabel labsize(1.2)) title(5 years postpartum, size(3))
...
xlabel(0(1)5, valuelabel labsize(1.2)) title(12 years postpartum, size(3))
...
xlabel(0(1)5, valuelabel labsize(1.2)) title(18 years postpartum, size(3))
...
xlabel(0(1)3, valuelabel labsize(1.5)) title(11 years postpartum, size(3))
...
xlabel(0(1)4, valuelabel labsize(1.5)) title(11 years postpartum, size(3))
